...
ModernUI.Config = {
    ...
    Skew       = 0.03,
    -- Corner radius in pixels. Works with or without Graphics/Glass: the
    -- corners are drawn with geometry when the texture is not installed.
    Radius     = 14,
...
-- When it is missing, the shapes below are drawn with geometry instead, so a
-- fresh checkout still gets rounded corners, shadows, sheen and rings.
...
-- ---------------------------------------------------------------------
-- Procedural shape backend
-- ---------------------------------------------------------------------
-- Modules/UI.Shapes.lua draws rounded panels, shadows, sheen and rings with
-- ActorMultiVertex geometry. It is pure Lua and ships with the theme, so it
-- is the reason none of the glass effects depend on generated PNGs anymore.
--
-- Loaded through pcall so that an engine without ActorMultiVertex, or a
-- checkout with the module deleted, degrades to square quads rather than
-- refusing to boot.
local shapesOk, shapesModule = pcall(LoadModule, "UI.Shapes.lua")
ModernUI.Shapes = (shapesOk and type(shapesModule) == "table") and shapesModule or nil
ModernUI.HasShapes = ModernUI.Shapes ~= nil
...
-- Returns 0 only when there is no way to draw a rounded corner at all, which
-- is what makes callers fall back to plain quads automatically. Either the
-- corner texture or the geometry backend is enough.
function ModernUI.Radius(width, height, requested)
    if not ModernUI.IsModern() then return 0 end
    if not (ModernUI.HasGlassArt or ModernUI.HasShapes) then return 0 end
...
--
-- Ring.png is a square texture, so stretching it to a strongly non-square
-- size distorts the corner radius. The geometry fallback does not have that
-- problem: it builds the outline from the requested width and height.
function ModernUI.FocusRing(params)
    params = params or {}
    local width = params.w or 96
    local height = params.h or 96

    if not ModernUI.HasRingArt then
        if not ModernUI.Shapes then return Def.Actor {} end

        local radius = params.radius or ModernUI.Radius(width, height)
        if radius <= 0 then radius = 8 end

        return ModernUI.Shapes.Ring {
            x = params.x, y = params.y,
            w = width, h = height,
            halign = params.halign, valign = params.valign,
            radius = radius,
            thickness = params.thickness or 2,
            color = Tint(params.color or ModernUI.Accent(1), params.alpha or 0.9),
        }
    end
...
--
-- Each layer picks the best backend available, in this order:
--   1. the Graphics/Glass texture, when installed (softest shadow)
--   2. ActorMultiVertex geometry from Modules/UI.Shapes.lua (exact corners)
--   3. a plain quad (square corners, always works)
--
...
function ModernUI.GlassCard(params)
    ...
    -- Ambient shadow. The pre-blurred sprite is much softer than geometry,
    -- and geometry is in turn much softer than a hard quad.
    ...
    elseif shapes then
        card[#card + 1] = shapes.SoftShadow {
            w = width, h = height,
            halign = halign, valign = valign,
            radius = radius > 0 and radius or 6,
            spread = 18,
            alpha = 0.4 * alpha,
        }
    else
    ...
    if radius > 0 and not ModernUI.HasGlassArt and shapes then
        -- Geometry path: one antialiased rounded panel with the gradient
        -- baked into its vertex colours. Exact at any width and height.
        card[#card + 1] = shapes.RoundedRect {
            w = width, h = height,
            halign = halign, valign = valign,
            radius = radius,
            topColor = topShade,
            bottomColor = bottomShade,
        }
    elseif radius > 0 then
        -- Nine-slice path: straight edges are quads, and the four corners are
        -- one rounded-corner sprite drawn four times at 0/90/180/270 degrees.
        ...
        -- Neither backend available: one quad, square corners, same gradient.
        card[#card + 1] = Def.Quad {
            InitCommand = function(self)
    ...
    -- Specular highlight. Additive and very faint, so the slight overhang past
    -- a rounded corner is not noticeable.
    if ModernUI.Config.Sheen and params.sheen ~= false then
        if ModernUI.HasSheenArt then
            card[#card + 1] = Def.Sprite {
        ...
        elseif shapes then
            -- Inset by the radius so the glint cannot poke out of a corner.
            card[#card + 1] = shapes.Sheen {
                w = width - radius, h = height - radius,
                halign = halign, valign = valign,
                color = ModernUI.Tokens.Text,
                alpha = params.sheenAlpha or 0.10,
            }
        end
    end
...
-- back to a glass card, which is upright but now properly rounded.
--
...
Trace("[Glassmorphism] Modern UI v2 loaded (style: " .. tostring(ModernUI.Config.Style)
    ...
    .. ", shapes: " .. tostring(ModernUI.HasShapes) .. ")")
